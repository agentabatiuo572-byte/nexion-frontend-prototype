...
const toMain = (sha) => `refs/heads/UniApp ${sha} refs/heads/UniApp ${ZERO}\n`;
...
test("删远端主线分支(local sha 全 0)不拦", () => {
  assert.equal(push(`(delete) ${ZERO} refs/heads/UniApp ${shaA}\n`).code, 0);
});
test("full 绿且树匹配 → 放行", () => {
  lastRun();
  const r = push(toMain(shaA));
...
test("端到端:core.hooksPath 指到 .githooks 后,真的 git push 会被 pre-push 拦(dry-run 也触发)", () => {
  ...
  execFileSync("git", ["init", "--bare", "-q", "-b", "UniApp", bare]);
  git("remote", "add", "origin", bare);
  mkdirSync(`${repo}/.githooks`, { recursive: true });
  for (const f of ["pre-push", "verify-before-push.mjs"]) writeFileSync(`${repo}/.githooks/${f}`, readFileSync(join(dirname(HOOK), f)));
  ...
  const r = spawnSync("git", ["-C", repo, "push", "--dry-run", "origin", "UniApp"], { encoding: "utf8" });
  ...
  const ok = spawnSync("git", ["-C", repo, "push", "--dry-run", "origin", "UniApp:refs/heads/pkg/free"], { encoding: "utf8" });
