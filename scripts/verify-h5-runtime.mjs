#!/usr/bin/env node
// H5 运行时门：只对本树固定 mock server 执行场景与直接探针。
//
// 包 ar(2026-08-17)两处扩展,行为不变、只加两个入口:
//   ① 复用 server：H5_RUNTIME_REUSE_MOCK_URL 指向 runner 已起的 server 时，
//      先核本树 + fixed mock 身份，通过才复用；不通过照旧自己起。
...
const reuseUrl = process.env.H5_RUNTIME_REUSE_MOCK_URL || null;
...
const server = await ensureServer({ root, reuseUrl, timeoutMs: 45_000, log: (m) => console.log(`h5-runtime: ${m}`) });
